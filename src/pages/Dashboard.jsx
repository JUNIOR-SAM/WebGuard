...
const NAV = [
  ...
  { label: 'New Scan', path: '/dashboard', icon: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
  ...
  return (
    <div style={{ backgroundColor: '#0a0e1a', minHeight: '100vh', display: 'flex', fontFamily: 'Geist, sans-serif', color: '#dfe2f3' }}>
      <style>{`
        ...
        @media (max-width: 600px) { .scan-form { flex-direction: column; } .stats-grid { grid-template-columns: repeat(2, 1fr) !important; } }
      `}</style>

      {/* Desktop Sidebar */}
      <nav className="desk-sidebar" style={{ width: '220px', minHeight: '100vh', backgroundColor: '#111827', borderRight: '1px solid #1f2937', display: 'flex', flexDirection: 'column', padding: '20px 12px', position: 'fixed', top: 0, left: 0, zIndex: 99 }}>
        ...
        <button onClick={() => setLogoutModal(true)}
          style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '13px', fontFamily: 'monospace', backgroundColor: 'transparent', color: '#bbcbb8', width: '100%' }}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
      ...
      <main className="main-area" style={{ marginLeft: '220px', flex: 1, padding: '24px', minHeight: '100vh', overflowY: 'auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <button style={{ backgroundColor: '#1f2937', border: '1px solid #374151', color: '#dfe2f3', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontFamily: 'monospace', display: 'flex', alignItems: 'center', gap: '6px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2fr) minmax(0, 1fr)', gap: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ backgroundColor: '#111827', border: '1px solid #1f2937', borderRadius: '12px', padding: '20px', position: 'relative', overflow: 'hidden' }}>
            ...
            <form onSubmit={handleScan} className="scan-form">
              <input type="url" placeholder="Enter website URL e.g. https://example.com" value={url} onChange={e => setUrl(e.target.value)} required
                style={{ flex: 1, backgroundColor: '#0a0e1a', border: '1px solid #374151', borderRadius: '8px', color: '#dfe2f3', padding: '11px 14px', fontSize: '13px', outline: 'none', fontFamily: 'monospace' }} />
            ...
            <p style={{ color: '#6b7280', fontSize: '11px', fontFamily: 'monospace', margin: '8px 0 0' }}>Target must be accessible via public internet. Internal IPs are restricted.</p>
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '8px' }}>
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
                  <path d="M23 6L13.5 15.5L8.5 10.5L1 18" stroke="#3fe56c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
        ...
        <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '16px' }}>
          {[
            { label: '● HIGH RISK FOUND', value: '12', color: '#ffb4ab', bg: 'rgba(255,180,171,0.05)', icon: '⚠' },
            { label: '● MEDIUM RISK FOUND', value: '45', color: 'orange', bg: 'rgba(255,165,0,0.05)', icon: '⊕' },
            { label: '● LOW RISK FOUND', value: '67', color: '#3fe56c', bg: 'rgba(63,229,108,0.05)', icon: 'ℹ' },
          ].map((s, i) => (
              ...
              <tbody>
                {recentScans.map((scan, i) => (
                  <tr key={i} style={{ borderTop: '1px solid #1f2937' }}>
                    <td style={{ padding: '13px 16px', color: '#dfe2f3', fontSize: '13px', fontFamily: 'monospace' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
                          <circle cx="12" cy="12" r="10" stroke="#6b7280" strokeWidth="1.5"/>
                          <path d="M2 12H22M12 2C9.33 5.33 8 8.67 8 12C8 15.33 9.33 18.67 12 22C14.67 18.67 16 15.33 16 12C16 8.67 14.67 5.33 12 2Z" stroke="#6b7280" strokeWidth="1.5"/>
                        </svg>
                        {scan.url}
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '13px 16px', textAlign: 'right' }}>
                      <button onClick={() => navigate('/results')} style={{ background: 'none', border: 'none', color: '#3fe56c', cursor: 'pointer', fontSize: '13px', fontFamily: 'monospace' }}>Details →</button>
                    </td>
                  </tr>
                ))}
              </tbody>
